...
const InputThankYouPage = () => {
  ...
  const [formErrors, setFormErrors] = useState({
    thankYouHeadline: false,
    thankYouDescription: false,
    thankYouCTA: false,
  });
  ...
  const handleChange = (e) => {
    ...
    if (e.target.name == "thankYouHeadline") {
      adcontext.setThankYouHeadline(e.target.value);
    }

    if (e.target.name == "thankYouCTA") {
      adcontext.setThankYouCTA(e.target.value);
    }

    if (e.target.name == "thankYouDescription") {
      adcontext.setThankYouDescription(e.target.value);
    }
    ...
    <form onSubmit={handleSubmit} className="max-w-lg mx-auto">
      <div className="mb-4">
        ...
        <div style={{ fontSize: "0.8rem", color: "#888" }}>
          Maximum {maxHeadline - formData.thankYouHeadline.length} characters
          remaining
        </div>
        {formErrors.thankYouHeadline && (
      ...
      <div className="mb-4">
        ...
        <input
          ...
          onChange={handleChange}
          className={`border rounded-lg py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline w-full ${
            formErrors.thankYouDescription
              ? "border-red-500"
              : "focus:border-indigo-500"
          }`}
        ...
        />
        <div style={{ fontSize: "0.8rem", color: "#888" }}>
          Maximum {maxDescription - formData.thankYouDescription.length}{" "}
      ...
      <div className="mb-4">
        ...
        <input
          type="text"
          name="thankYouCTA"
          value={formData.thankYouCTA}
          maxLength={maxCTA}
          onChange={handleChange}
          className={`border rounded-lg py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline w-full ${
            formErrors.thankYouCTA
              ? "border-red-500"
              : "focus:border-indigo-500"
          }`}
        ...
        />
        <div style={{ fontSize: "0.8rem", color: "#888" }}>
          Maximum {maxCTA - formData.thankYouCTA.length} characters remaining
        </div>
        {formErrors.thankYouCTA && (
          <p className="text-red-500 text-sm mt-1">This field is required</p>
        )}
...
export default InputThankYouPage;
